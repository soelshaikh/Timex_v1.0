...
const client = require("twilio")(
  ...
  "39d680ece88bb5e5d02001556a207992"
);
...
router.post("/sendotp", async (req, res) => {
  ...
  try {
    ...
    if (preuser) {
      ...
      if (existEmail) {
        const updateData = await userotpdb.findByIdAndUpdate(
          { _id: existEmail._id },
          {
            otp: OTP,
          },
      ...
      res
        .status(400)
        .json({ status: 400, error: "This User Not Exist In our Db" });
    }
  } catch (error) {
...
router.get("/getactiveproduct", async (req, res) => {
  ...
  try {
    const products = await productdb
      .find({ isActive: true, product_name: { $regex: search, $options: "i" } })
      .populate("product_category");
...
router.post("/makeorder/:id", authenticate, async (req, res) => {
  ...
  try {
    ...
    for (var i = 0; i < user.carts.length; i++) {
      totalPrice +=
        user.carts[i].product[0].selling_price * user.carts[i].quantity;
    }
    ...
    if (user.carts.length > 0) {
      ...
      for (let index = 0; index < user.carts.length; index++) {
        ...
        //   user.carts[index].product[0]._id,
        //   {
        //     countInStock:
        //       user.carts[index].product[0].countInStock -
    ...
    const validUserOne = await userdb.findOne({ _id: req.userId });
    const allOrders = await orderdb
      .find({ user: req.userId })
      .sort({ _id: -1 })
      .limit(1);

    console.log("allOrders");
    console.log(allOrders._id);
    ...
    client.messages
      ...
      Order Details:
    - Order ID: ${allOrders[0]._id}
      - Customer Name: ${address.fname + " " + address.lname}
      - Order Total: ${allOrders[0].totalPriceAfterDiscounts}
      - Delivery Address: ${
      ...
      - Expected Delivery Date: ${allOrders[0].orderDate}

      We will process your order as soon as possible. If you have any questions or concerns, please feel free to contact us.

      Thank you for choosing our service! 😊`,
        from: "whatsapp:+14155238886",
        to: `whatsapp:+91${address.phone}`,
      })
        ...
        if (validUserOne) {
          if (wbalance > 0) {
            ...
            userdb.findByIdAndUpdate(
              ...
              {
                $push: {
                  balanceHistory: { Type: "Debited", Amount: wbalance },
                },
              },
              { safe: true, upsert: true, new: true },
  ...
  try {
    ...
    if (Orders != "") {
      ...
      for (let index = 0; index < Totalsell.length; index++) {
        TotalsellAfterDiscounts =
          TotalsellAfterDiscounts + Totalsell[index].totalPriceAfterDiscounts;
      }
      ...
      for (let index = 0; index < Totalsellb.length; index++) {
        TotalsellBeforeDiscounts =
          TotalsellBeforeDiscounts + Totalsellb[index].totalPrice;
      }
      ...
      const ProductCount = {};
      Name_array.forEach(function (x) {
        ProductCount[x] = (ProductCount[x] || 0) + 1;
      });
      console.log(ProductCount);

      const CoupanCount = {};
      coupan_array.forEach(function (x) {
        CoupanCount[x] = (CoupanCount[x] || 0) + 1;
      });
      console.log(CoupanCount);

      const order_statusCount = {};
      orderStatus_array.forEach(function (x) {
        order_statusCount[x] = (order_statusCount[x] || 0) + 1;
      });
      console.log(order_statusCount);

      const PaymentStatus_count = {};
      PaymentStatus_array.forEach(function (x) {
        PaymentStatus_count[x] = (PaymentStatus_count[x] || 0) + 1;
      });
      console.log(PaymentStatus_count);

      const orderDate_count = {};
      Date_array.forEach(function (x) {
        orderDate_count[x] = (orderDate_count[x] || 0) + 1;
      });
      ...
      res.status(201).json({
        status: 201,
        Orders: Orders,
        TotalsellBeforeDiscounts: TotalsellBeforeDiscounts,
        TotalsellAfterDiscounts: TotalsellAfterDiscounts,
        ProductCount: ProductCount,
        CoupanCount: CoupanCount,
        order_statusCount: order_statusCount,
        PaymentStatus_count: PaymentStatus_count,
        orderDate_count: orderDate_count,
      });
    } else {
...
router.get("/getweeksorders", async (req, res) => {
  try {
    ...
    const Orders = await orderdb.find({
      orderDate: { $gt: weekago, $lt: TodaysDate },
    });
    ...
    if (Orders != "") {
      ...
      //Total sell Identify
      var TotalsellAfterDiscounts = 0;
      const Totalsell = await orderdb.find({
        orderDate: { $gt: weekago, $lt: TodaysDate },
      });
      // console.log(Totalsell);
      console.log(Totalsell[0].totalPriceAfterDiscounts);
      ...
      var TotalsellBeforeDiscounts = 0;
      const Totalsellb = await orderdb.find({
        orderDate: { $gt: weekago, $lt: TodaysDate },
      });
      // console.log(Totalsell);
      console.log(Totalsellb[0].totalPrice);
...
router.get("/getmonthlyorders", async (req, res) => {
  try {
    ...
    const Orders = await orderdb.find({
      orderDate: { $gt: mago, $lt: TodaysDate },
    });
    console.log(Orders.length);
    if (Orders != "") {
      let flag = 0;
...
router.get("/getspecificdatesorder", async (req, res) => {
  try {
    ...
    if (Orders != "") {
      ...
      var TotalsellAfterDiscounts = 0;
      const Totalsell = await orderdb.find(
        { orderDate: { $gte: sda, $lte: eda } },
        { totalPriceAfterDiscounts: 1, _id: 0 }
      );
      // console.log(Totalsell);
      console.log(Totalsell[0].totalPriceAfterDiscounts);
      for (let index = 0; index < Totalsell.length; index++) {
...
router.get("/returnorder/:id", authenticate, async (req, res) => {
  try {
    ...
    const user = await userdb.findById({ _id: req.userId });
    const updateBalance = await userdb.findByIdAndUpdate(
      { _id: user._id },
      {
...
router.post("/addcoupan", async (req, res) => {
  ...
  try {
    ...
    if (precoupan) {
      ...
      res
        .status(424)
        .json({ status: 424, error: "This Coupan name is Already Exist !" });
    } else {
...
router.delete("/deletecoupans", async (req, res) => {
  try {
    const deletecoupan = await coupandb.remove({});
