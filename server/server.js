...
nodeCron.schedule("57 14 * * *", async () => {
  ...
  for (let index = 0; index < preuser.length; index++) {
    if (preuser[index].dateOfBirth != undefined) {
      ...
      if (
        moment(preuser[index].dateOfBirth).format("DD-MM") ==
        moment().format("DD-MM")
      ) {
        const validUserOne = await userdb.find(
          { email: preuser[index].email },
          { balance: 1, _id: 1 }
        );
        for (let index = 0; index < validUserOne.length; index++) {
          ...
          userdb.findByIdAndUpdate(
            ...
            { balance: validUserOne[index].balance + 200 },
            function (err, docs) {
              if (err) {
                console.log(err);
              } else {
...
// nodeCron.schedule("07 16 * * *", async () => {
//   console.log("start");
//   const accountSid = "AC8932bdddad0bb6549ae8ba6700149d91";
//   const authToken = "39d680ece88bb5e5d02001556a207992";
//   const client = require("twilio")(accountSid, authToken);

//   client.messages
//     .create({
//       body: "order done Timex",
//       from: "whatsapp:+14155238886",
//       to: "whatsapp:+919104744430",
//     })
